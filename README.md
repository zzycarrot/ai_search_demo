...
* **⚡️ 增量索引**:
  * **元数据快速检测**: 基于文件大小 (file_size) 和修改时间 (mtime) 快速判断文件变化，无需读取内容。
  * **极速启动**: 重启时仅处理新增或变更的文件，未变化的文件直接跳过。
  * **孤儿清理**: 启动时自动清理已删除文件的残留索引和缓存。
...
**解决方案**: 内容哈希校验 + 元数据缓存
```rust
// Embedding 缓存: key = 文件路径, value = (content_hash, keywords)
...
// 元数据缓存: key = 文件路径, value = (file_size, mtime)
// 启动扫描时先检查元数据，避免读取文件内容
match cache.check_file_status(&path) {
    FileStatus::Unchanged => return, // 跳过
    FileStatus::New => println!("[新增]"),
    FileStatus::Modified => println!("[变更]"),
}
```
...
fn cleanup_orphan_indexes() {
    ...
    for doc in index.all_documents() {
        ...
        if !Path::new(path).exists() {
            ...
            cache.remove(path);
            cache.remove_file_meta(path);
        }
    }
    // 清理元数据缓存中的孤儿记录
    for path in cache.get_all_cached_paths() {
        if !Path::new(&path).exists() {
            cache.remove(&path);
            cache.remove_file_meta(&path);
        }
...
| `FileMetaCache` | sled 内置锁 | 文件元数据缓存 |
...
2. **扫描阶段**
   * 遍历 `docs/` 目录（支持 ripgrep 风格，respect .gitignore）。
   * **元数据快速检测**: 比对 file_size 和 mtime，无需读取文件内容。
   * 通过 Registry 原子获取文件处理权。
   * 跳过未变更文件（显示 `[新增]` 或 `[变更]` 状态）。
   * 清理孤儿索引和孤儿元数据缓存。
